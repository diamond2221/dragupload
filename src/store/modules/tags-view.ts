...
import { getViewTags } from '@/utils/storage'
...
export interface ITagsViewState {
  ...
  willColseTag: ITagView | null
}

@Module({ dynamic: true, store, name: 'tagsView' })
class TagsView extends VuexModule implements ITagsViewState {
  public visitedViews: ITagView[] = getViewTags()
  public cachedViews: (string | undefined)[] = []
  public willColseTag: ITagView | null = null

  @Mutation
  private SET_WILLCOLSETAG(tag: ITagView | null) {
    this.willColseTag = tag
  }
  ...
  /**
   *
   * @param tag 离开当前页面，并且销毁关闭当前标签
   */
  @Action
  public setWillColseTag(tag: ITagView | null) {
    if (tag && !tag.title) {
      tag.title = tag.meta.title
    }
    this.SET_WILLCOLSETAG(tag)
  }
  ...
  @Action
  public delVistedView(view: ITagView) {
